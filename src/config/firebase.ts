...
// Default Demo / Public Firebase Config or Environment Variables
...
try {
  ...
  // Only connect firestore if configured with real project
  if (import.meta.env.VITE_FIREBASE_PROJECT_ID) {
    db = getFirestore(app);
    isFirebaseReady = true;
  }
} catch (e) {
  console.warn("Firebase not configured with active cloud credentials, falling back to real-time BroadcastChannel / local state engine:", e);
}

// Real-time synchronization layer (Firestore + Cross-Tab/Device BroadcastChannel Engine)
const broadcast = typeof window !== 'undefined' ? new BroadcastChannel('zhuazhou_realtime_protocol') : null;

// Initial mock state with authentic guests for a lively party feel
const DEFAULT_GUESTS: GuessRecord[] = [
  { id: 'g_1', name: '舅舅 Howard', selections: ['item_09', 'item_16', 'item_11'], timestamp: Date.now() - 1000 * 60 * 15, avatarSeed: 1 },
  { id: 'g_2', name: '阿嬤 (Grandma)', selections: ['item_09', 'item_15', 'item_04'], timestamp: Date.now() - 1000 * 60 * 12, avatarSeed: 2 },
  { id: 'g_3', name: '阿公 (Grandpa)', selections: ['item_11', 'item_10', 'item_06'], timestamp: Date.now() - 1000 * 60 * 10, avatarSeed: 3 },
  { id: 'g_4', name: '姑姑 Emily', selections: ['item_01', 'item_08', 'item_02'], timestamp: Date.now() - 1000 * 60 * 8, avatarSeed: 4 },
  { id: 'g_5', name: '乾爹 Ken', selections: ['item_16', 'item_03', 'item_05'], timestamp: Date.now() - 1000 * 60 * 5, avatarSeed: 5 },
  { id: 'g_6', name: '姨婆 May', selections: ['item_04', 'item_09', 'item_13'], timestamp: Date.now() - 1000 * 60 * 2, avatarSeed: 6 },
];

const LOCAL_STORAGE_KEY_GUESSES = 'xingwei_zhuazhou_guesses';
const LOCAL_STORAGE_KEY_GAME = 'xingwei_zhuazhou_gamestate';
  ...
  try {
    ...
    if (!raw) {
      const initial: GameState = {
        isRevealed: false,
        actualItems: ['item_09', 'item_16', 'item_01'], // Default picked if triggered
      };
...
export const submitGuessToDb = async (name: string, selections: string[]): Promise<void> => {
  ...
  if (isFirebaseReady && db) {
    try {
      ...
      console.warn("Firestore write error, updating local sync layer:", err);
    }
  }
...
export const subscribeToGuesses = (callback: (guesses: GuessRecord[]) => void) => {
  ...
  if (isFirebaseReady && db) {
    try {
      const q = query(collection(db, 'guesses'), orderBy('timestamp', 'desc'), limit(100));
  ...
  // Fallback / Realtime Broadcast Sync
...
// API: Reset all guesses (for admin party reset)
